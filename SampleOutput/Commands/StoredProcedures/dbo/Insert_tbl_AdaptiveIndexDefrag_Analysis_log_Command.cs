...
{
    public record Insert_tbl_AdaptiveIndexDefrag_Analysis_log_Command(string? Operation, int dbID, string? dbName, int objectID, string? objectName, int index_or_stat_ID, short? partitionNumber, DateTime? dateTimeStart, DateTime? dateTimeEnd, int durationSeconds, string? errorMessage) : IDatabaseCommand
    {
        ...
        public string GetSqlStatement() => "[dbo].[Insert_tbl_AdaptiveIndexDefrag_Analysis_log]";
        ...
        {
            return $"Operation:{Operation}, dbID:{dbID}, dbName:{dbName}, objectID:{objectID}, objectName:{objectName}, index_or_stat_ID:{index_or_stat_ID}, partitionNumber:{partitionNumber}, dateTimeStart:{dateTimeStart}, dateTimeEnd:{dateTimeEnd}, durationSeconds:{durationSeconds}, errorMessage:{errorMessage}";
        }
    }
}