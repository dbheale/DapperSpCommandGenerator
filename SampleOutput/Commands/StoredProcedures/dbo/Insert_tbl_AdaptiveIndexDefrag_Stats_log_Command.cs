...
{
    public record Insert_tbl_AdaptiveIndexDefrag_Stats_log_Command(int dbID, string? dbName, int objectID, string? objectName, int statsID, string? statsName, short? partitionNumber, long? rows, long? rows_sampled, long? modification_counter, bool? no_recompute, DateTime? dateTimeStart, DateTime? dateTimeEnd, int durationSeconds, string? sqlStatement, string? errorMessage) : IDatabaseCommand
    {
        ...
        public string GetSqlStatement() => "[dbo].[Insert_tbl_AdaptiveIndexDefrag_Stats_log]";
        ...
        {
            return $"dbID:{dbID}, dbName:{dbName}, objectID:{objectID}, objectName:{objectName}, statsID:{statsID}, statsName:{statsName}, partitionNumber:{partitionNumber}, rows:{rows}, rows_sampled:{rows_sampled}, modification_counter:{modification_counter}, no_recompute:{no_recompute}, dateTimeStart:{dateTimeStart}, dateTimeEnd:{dateTimeEnd}, durationSeconds:{durationSeconds}, sqlStatement:{sqlStatement}, errorMessage:{errorMessage}";
        }
    }
}