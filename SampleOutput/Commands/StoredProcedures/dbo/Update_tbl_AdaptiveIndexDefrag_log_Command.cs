...
{
    public record Update_tbl_AdaptiveIndexDefrag_log_Command(int indexDefrag_id, int dbID, string? dbName, int objectID, string? objectName, int indexID, string? indexName, short? partitionNumber, double? fragmentation, long? page_count, long? range_scan_count, int fill_factor, DateTime? dateTimeStart, DateTime? dateTimeEnd, int durationSeconds, string? sqlStatement, string? errorMessage) : IDatabaseCommand
    {
        public DynamicParameters GetParameters()
        {
            var p = new DynamicParameters();
            p.Add("indexDefrag_id", indexDefrag_id);
			p.Add("dbID", dbID);
			p.Add("dbName", dbName);
        ...
        public string GetSqlStatement() => "[dbo].[Update_tbl_AdaptiveIndexDefrag_log]";
        ...
        {
            return $"indexDefrag_id:{indexDefrag_id}, dbID:{dbID}, dbName:{dbName}, objectID:{objectID}, objectName:{objectName}, indexID:{indexID}, indexName:{indexName}, partitionNumber:{partitionNumber}, fragmentation:{fragmentation}, page_count:{page_count}, range_scan_count:{range_scan_count}, fill_factor:{fill_factor}, dateTimeStart:{dateTimeStart}, dateTimeEnd:{dateTimeEnd}, durationSeconds:{durationSeconds}, sqlStatement:{sqlStatement}, errorMessage:{errorMessage}";
        }
    }
}