...
{
    public record Update_RepositoryFiles_Command(long? Id, int ReferenceId, int ReferenceTypeId, int StorageTypeId, string? FileName, string? FileExtension, int CreatorUserId, DateTime? DateCreated, long? FileSize) : IDatabaseCommand
    {
        ...
        public string GetSqlStatement() => "[dbo].[Update_RepositoryFiles]";
        ...
        {
            return $"Id:{Id}, ReferenceId:{ReferenceId}, ReferenceTypeId:{ReferenceTypeId}, StorageTypeId:{StorageTypeId}, FileName:{FileName}, FileExtension:{FileExtension}, CreatorUserId:{CreatorUserId}, DateCreated:{DateCreated}, FileSize:{FileSize}";
        }
    }
}