...
{
    public record usp_AdaptiveIndexDefrag_PurgeLogs_Command(short? daystokeep) : IDatabaseCommand
    {
        public DynamicParameters GetParameters()
        {
            var p = new DynamicParameters();
            p.Add("daystokeep", daystokeep);
            return p;
        }

        public CommandType GetCommandType() => CommandType.StoredProcedure;

        public string GetSqlStatement() => "[dbo].[usp_AdaptiveIndexDefrag_PurgeLogs]";
