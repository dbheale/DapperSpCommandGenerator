...
{
    public record Get_WorkOrderByReferenceNumber_Command(string? ReferenceNumber) : IDatabaseCommand
    {
        public DynamicParameters GetParameters()
        {
            var p = new DynamicParameters();
            p.Add("ReferenceNumber", ReferenceNumber);
            return p;
        }

        public CommandType GetCommandType() => CommandType.StoredProcedure;

        public string GetSqlStatement() => "[dbo].[Get_WorkOrderByReferenceNumber]";
